...
function renderContent(html, data = {}) {
  const { settings = {}, faqs = [], testimonials = [], services = [], brands = [],
          skills = [], counters = [], isHome = false, work = null } = data;
  ...
  // These pages were shipping ~50 words of crawlable text: the title was injected
  // into <head> but the write-up itself only appeared after js/dynamic.js ran. So
  // the strongest content on the site — the pages a prospective client reads
  // hardest, and the ones matchProjects sends people to — were invisible to search
  // engines and to every AI crawler, which do not execute JavaScript.
  if (work) {
    setText(doc.querySelector('.work-details-name'), work.title);
    setText(doc.querySelector('.work-details-summery'), work.description);
    setText(doc.querySelector('.work-details-category'), work.category || 'Project');
    setImage(doc.querySelector('.work-details-image'), work.thumbnail_url,
      `${work.title} — case study by Jomiez Innovation`);
    setText(doc.querySelector('.work-project-name'), work.title);

    const info = doc.querySelectorAll('.work-details-info-text.white');
    const fields = [work.date, work.category, work.client, work.project_link];
    info.forEach((el, i) => {
      const v = fields[i];
      if (v) setText(el, i === 3 ? String(v).replace(/^https?:\/\//, '').replace(/\/$/, '') : v);
    });

    // content is trusted author HTML from the CMS, so it goes in as markup —
    // the same value dynamic.js assigns to innerHTML on the client.
    const body = doc.querySelector('.work-project-except');
    if (body && (work.content || work.description)) {
      body.classList.remove('skeleton');
      body.innerHTML = work.content || `<p>${work.description}</p>`;
    }
    stats.work = work.title;
  }
