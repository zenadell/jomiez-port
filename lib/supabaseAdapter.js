...
class SupabaseAdapter {
  constructor() {
    ...
    let connStr = process.env.SUPABASE_DATABASE_URL || process.env.DATABASE_URL || '';
  ...
  convertSql(sql) {
    ...
    // Replace ? placeholders with $1, $2, ... but ONLY outside string literals.
    // A naive /\?/g also rewrites question marks inside quoted text, which silently
    // corrupts any content containing one (e.g. an FAQ question saved as "...offer$1").
    let index = 1;
    let out = '';
    let quote = null; // active quote char, or null when outside a literal
    for (let i = 0; i < newSql.length; i++) {
      const c = newSql[i];
      if (quote) {
        out += c;
        // '' and "" are escaped quotes inside a literal, not a terminator
        if (c === quote) {
          if (newSql[i + 1] === quote) { out += newSql[++i]; } else { quote = null; }
        }
      } else if (c === "'" || c === '"') {
        quote = c; out += c;
      } else if (c === '?') {
