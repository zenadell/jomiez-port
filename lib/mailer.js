...
async function domainAcceptsMail(domain) {
  ...
  try {
    ...
    // ENODATA is the important one and the easiest to get wrong: the domain
    // resolves, it just publishes no MX. ghremodeling.com answers that way — it
    // has a web server and no mail server — and every message to it bounced.
    // SERVFAIL or a timeout says nothing about the domain, so those pass.
    verdict = ['ENODATA', 'ENOTFOUND', 'NXDOMAIN'].includes(e.code)
      ? { ok: false, reason: `${key} publishes no mail server, so nothing sent there can arrive` }
      : { ok: true };
  }
