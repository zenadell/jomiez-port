 ...
 *
 * Checked inside the sender rather than at each call site, because there are
 * three of those and a fourth will be added eventually. The footer promises
 * "I will not contact you"; this is what makes that true.
 */
let suppressionLookup = null;
function setSuppressionLookup(fn) { suppressionLookup = fn; }
...
async function sendLeadReply({ to, subject, body, replyTo, dryRun = false, skipMxCheck = false }) {
  ...
  if (suppressionLookup) {
    const hit = await suppressionLookup(address);
    if (hit) {
      return { sent: false, suppressed: true,
               reason: `Not sent — this address asked not to be contacted (${hit}).` };
    }
  }
  if (!skipMxCheck) {
...
module.exports = { isConfigured, missingConfig, sendLeadReply, notifyOwner, domainAcceptsMail, setSuppressionLookup };
