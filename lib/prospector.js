...
/**
 * Pulls real contact addresses out of a page.
 *
 * Naively regexing for @ finds plenty of addresses that belong to nobody you can
 * write to: Sentry ingest keys, analytics endpoints, and the licence headers
 * that font foundries embed in CSS. An earlier shortlist put a Sentry address
 * and a type foundry's support desk forward as prospect contacts, which is the
 * kind of mistake that only shows up once the email has already gone.
 */
const EMAIL_NOISE = /@(sentry|.*\.sentry|wixpress|sentry-next\.wixpress|.*\.ingest\.sentry|example|domain|email|yourdomain|test|localhost|placeholder)\b/i;
const NOISE_DOMAINS = /(wixpress\.com|sentry\.io|indiantypefoundry\.com|fontspring\.com|myfonts\.com|fonts\.com|typography\.com|monotype\.com|linotype\.com|adobe\.com|w3\.org|schema\.org|googlegroups\.com|wordpress\.org|squarespace\.com|godaddy\.com|sentry\.wixpress\.com)$/i;
const NOISE_LOCAL = /^(no-?reply|donotreply|postmaster|abuse|webmaster@localhost|sentry|example|your|email|name|user|username|firstname|lastname|someone|test|demo|admin@example)/i;

function harvestEmails(html) {
  return [...new Set((html.match(/[a-z0-9._%+-]+@[a-z0-9.-]+\.[a-z]{2,}/gi) || [])
    .map(e => e.trim())
    // "mailto:%20info@..." is common in hand-written HTML, and % is legal in the
    // pattern above, so the encoded space was captured as part of the address and
    // sent as %20info@... — which bounced off a mailbox that exists perfectly well.
    .map(e => {
      const decoded = e.replace(/%[0-9a-f]{2}/gi, ' ').trim();
      const tail = decoded.split(/\s+/).pop();      // keep only the address itself
      return tail || '';
    })
    .filter(e => e && !e.includes('%'))
    // "APR-240517-CO-300x163@2x.avif" reached a prospect as a recipient: modern
    // image formats were missing from this list, and a filename containing @2x
    // matches the address pattern perfectly. Reject by extension and by the
    // tell-tale pixel dimensions rather than trying to enumerate every format.
    .filter(e => !/\.(png|jpe?g|jpg|gif|svg|webp|avif|heic|bmp|ico|tiff?|css|js|mjs|json|woff2?|ttf|eot|mp4|webm|pdf)$/i.test(e))
    .filter(e => !/\b\d{2,4}x\d{2,4}\b/.test(e))
    .filter(e => !/@\d+x\./i.test(e))
    .filter(e => !EMAIL_NOISE.test(e))
    .filter(e => !NOISE_DOMAINS.test(e.split('@')[1] || ''))
    .filter(e => !NOISE_LOCAL.test(e.split('@')[0] || ''))
...
async function findContactEmail(signals) {
  ...
  for (const path of ['/contact', '/contact-us', '/contactus', '/about']) {
    try {
      ...
      const found = harvestEmails(got.html);
...
async function analyse(url, geminiKey, ourWork = []) {
  ...
  // Look at the page, not just its HTML. This is where the arguments worth
  // four figures come from.
  let visual = null, visualNote = null;
  if (geminiKey) {
    const shot = await captureScreenshot(signals.url, geminiKey);
    if (!shot.ok) {
      visualNote = `screenshot: ${shot.reason}`;
    } else {
      const v = await visualCritique(shot, geminiKey, { name: guessName(signals), industry: '' });
      if (v.ok) visual = { ...v, source: shot.source, metrics: shot.metrics || null };
      else visualNote = `vision: ${v.reason}`;
    }
  } else {
    visualNote = 'no gemini key';
  }
...
/**
 * Renders the homepage and asks a vision model what it looks like.
 *
 * This exists because a text-only audit produces reasons nobody buys on. The
 * strongest thing measurable in HTML was "your phone number is not a tel: link",
 * and no owner spends four figures over that. Looking at the page finds the
 * arguments that actually land — a Google+ icon still in the header of a law
 * firm's site, seven years after Google+ shut down, says "nobody has touched
 * this" far better than any tag ever could.
 */

/**
 * Gets a picture of the homepage, and where possible Google's own verdict on it.
 *
 * Order matters. PageSpeed Insights runs Lighthouse on Google's infrastructure
 * and hands back both a rendered screenshot and real mobile performance numbers
 * — and "Google scores your site 24 out of 100 on a phone" is a harder argument
 * than anything we could assert ourselves. It needs an API key.
 *
 * thum.io is the keyless fallback. It renders fine from a laptop but refuses
 * Render's datacenter address, so it is last rather than first.
 */
const pct = (cat) => (cat && typeof cat.score === 'number') ? Math.round(cat.score * 100) : null;

async function fromPageSpeed(url, key) {
  // One request, four categories. Google scoring their accessibility or SEO out
  // of 100 is the same kind of undeniable third-party fact as the speed score,
  ...
  try {
    ...
    if (data.error) return { ok: false, reason: `PageSpeed ${res.status}: ${String(data.error.message).slice(0, 120)}` };

    const lr = data.lighthouseResult || {};
    const audits = lr.audits || {};
    const raw = (audits['final-screenshot'] || {}).details || {};
    const dataUri = raw.data || '';
    const m = dataUri.match(/^data:(image\/[a-z]+);base64,(.+)$/i);
    if (!m) return { ok: false, reason: 'PageSpeed returned no screenshot.' };
    ...
    return {
      ...
      source: 'pagespeed',
      metrics: {
        score: pct((lr.categories || {}).performance),
        accessibility: pct((lr.categories || {}).accessibility),
        seo: pct((lr.categories || {}).seo),
        bestPractices: pct((lr.categories || {})['best-practices']),
        lcp: (audits['largest-contentful-paint'] || {}).displayValue,
        cls: (audits['cumulative-layout-shift'] || {}).displayValue,
        tbt: (audits['total-blocking-time'] || {}).displayValue,
        fcp: (audits['first-contentful-paint'] || {}).displayValue,
        weight: (audits['total-byte-weight'] || {}).displayValue,
        // Mobile usability failures a phone user feels immediately.
        tapTargetsFail: (audits['tap-targets'] || {}).score === 0,
        tinyText: (audits['font-size'] || {}).score === 0,
        notHttps: (audits['is-on-https'] || {}).score === 0
      }
    };
  } catch (e) {
    return { ok: false, reason: `PageSpeed: ${e.name === 'TimeoutError' ? 'timed out' : e.message}` };
  }
}

// Serves an animated GIF placeholder while it renders, so the content type is
// what tells us whether the render actually finished.
async function fromThumIo(url, attempts = 2) {
  const target = `https://image.thum.io/get/width/1000/crop/1400/noanimate/${url}`;
  let last = 'no response';
  for (let i = 0; i < attempts; i++) {
    try {
      const res = await fetch(target, { signal: AbortSignal.timeout(45000) });
      const type = (res.headers.get('content-type') || '').toLowerCase();
      if (res.ok && /image\/(png|jpe?g)/.test(type)) {
        const buf = Buffer.from(await res.arrayBuffer());
        if (buf.length > 15000) {
          return { ok: true, base64: buf.toString('base64'), source: 'thum.io',
                   mime: type.includes('png') ? 'image/png' : 'image/jpeg' };
        }
        last = `returned ${buf.length} bytes`;
      } else {
        last = `HTTP ${res.status} ${type || 'no type'}`;
      }
    } catch (e) { last = e.name === 'TimeoutError' ? 'timed out' : e.message; }
    if (i < attempts - 1) await new Promise(r => setTimeout(r, 9000));
  }
  return { ok: false, reason: `thum.io ${last}` };
}

async function captureScreenshot(url, geminiKey) {
  const tried = [];
  // A dedicated key first, then the Gemini key — an AI Studio key often belongs
  // to a project where the PageSpeed API can simply be switched on.
  for (const key of [process.env.PAGESPEED_API_KEY, geminiKey, null]) {
    const got = await fromPageSpeed(url, key);
    if (got.ok) return got;
    tried.push(got.reason);
    if (!/403|401|quota|API key|not been used|disabled/i.test(got.reason)) break;
  }
  const fallback = await fromThumIo(url);
  if (fallback.ok) return fallback;
  tried.push(fallback.reason);
  return { ok: false, reason: tried.join(' | ').slice(0, 300) };
}

/** What a prospective customer sees in the first three seconds. */
async function visualCritique(shot, geminiKey, ctx) {
  const client = new GoogleGenerativeAI(geminiKey);
  let lastErr = null;
  const prompt = `You are a senior web designer looking at a screenshot of ${ctx.name || 'a business'}'s homepage${ctx.industry ? ` (${ctx.industry})` : ''}.

Judge it the way a prospective customer would in the first three seconds, and the
way a designer would when deciding whether it needs a rebuild rather than a tidy-up.

FIRST, AND SEPARATELY FROM EVERYTHING ELSE — how old does it LOOK?

A site can have every modern ingredient and still look ten years out of date.
Google reviews, booking buttons, award badges and trust seals are features, not
design. Judge the visual language on its own: a shop with 876 reviews on display
and a booking button can still be wearing a 2013 template, and that is exactly
the case worth telling an owner about, because they believe their site is fine.

Dated visual language, whatever features are present:
glossy or gradient logos, drop shadows and bevels, starburst or ribbon award
badges clustered together, saturated full-width colour bars, stock photography
of smiling models, text laid over a busy photograph, cramped navigation with
many small links, small pill buttons with gradients, tight line spacing, little
whitespace, three or more competing accent colours, a carousel.

Current design language, 2025 onward:
generous whitespace, a large confident type scale, a restrained palette of two
or three colours, real photography of the actual place and people, one obvious
primary action, flat or very soft depth, calm and uncrowded.

Then look for and report only what you can actually SEE:
- What era the design is from, and what specifically dates it (layout style, fonts,
  gradients, stock photography, button styling, spacing).
- Dead or obsolete elements: a Google+ icon, a Flash notice, "best viewed in",
  Twitter's old bird, a copyright year in the past, broken or missing images.
- Whether it is instantly clear what this business does and where, or whether the
  visitor has to work it out.
- Whether there is any visible reason to trust them: real photographs of real
  people or premises, reviews, credentials, named staff — or only stock imagery.
- Whether the most important action (call, book, get a quote) is visible without
  scrolling, and whether it looks like a button a person would press.
- Overall impression: does this look like a business that is thriving, or one that
  might have closed?

Return strict JSON, nothing else:
{
  "looks_current": true or false — does the VISUAL DESIGN look like it was made in the last two years, ignoring what features it has,
  "era_verdict": "one sentence naming the era the design belongs to and the specific visual choices that place it there",
  "design_era": "e.g. 'mid-2010s WordPress theme' — be specific about what dates it",
  "first_impression": "one sentence on what a customer concludes in three seconds",
  "dated_signals": ["concrete visible things that make it look old or neglected, strongest first"],
  "trust_gap": "one sentence on what is missing that would make a stranger trust them",
  "clarity_problem": "one sentence — is it obvious what they do and who for, or not",
  "strongest_argument": "the single most persuasive sentence you could say to this owner about why their site needs replacing, written as plain speech, naming something visible in the screenshot. If the site works but looks dated, this sentence should be about how old it looks and what that costs them against a competitor whose site looks current."
}

Never invent anything you cannot see. If the design genuinely looks current —
made in the last two years, not merely functional — say so plainly and set
looks_current to true rather than manufacturing faults.

But do not confuse "has the right features" with "looks current". If the site
works well and has everything it needs yet the styling belongs to an earlier
decade, say exactly that: it is a real finding, it is the most common case among
established businesses, and it is the most useful thing you can tell an owner who
already believes their site is fine.`;

  for (const model of ['gemini-3.5-flash-lite', 'gemini-flash-latest', 'gemini-2.5-flash']) {
    try {
      const out = await client.getGenerativeModel({ model }).generateContent([
        { text: prompt },
        { inlineData: { mimeType: shot.mime, data: shot.base64 } }
      ]);
      const raw = out.response.text().replace(/^```(?:json)?|```$/gm, '').trim();
      const parsed = JSON.parse(raw);
      if (parsed && parsed.strongest_argument) return { ok: true, ...parsed };
    } catch (e) { lastErr = e; }
  }
  return { ok: false, reason: `Vision analysis failed: ${lastErr ? String(lastErr.message).slice(0, 160) : 'no model returned usable JSON'}` };
}

module.exports.captureScreenshot = captureScreenshot;
module.exports.visualCritique = visualCritique;
