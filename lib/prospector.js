...
// thum.io renders on demand and serves an animated GIF placeholder while it
// works, so the content type is what tells us whether the render finished.
async function captureScreenshot(url, attempts = 3) {
  ...
  for (let i = 0; i < attempts; i++) {
    try {
      ...
      if (res.ok && /image\/(png|jpe?g)/.test(type)) {
        ...
        if (buf.length > 15000) {
          return { ok: true, base64: buf.toString('base64'), mime: type.includes('png') ? 'image/png' : 'image/jpeg' };
        }
      }
    } catch (e) { /* fall through to the retry */ }
    if (i < attempts - 1) await new Promise(r => setTimeout(r, 9000));
  }
  return { ok: false, reason: 'Could not render a screenshot of that page.' };
}
