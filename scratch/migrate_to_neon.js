...
const neonAdapter = require(path.join(__dirname, '../lib/neonAdapter'));
const fs = require('fs');

async function migrate() {
    console.log('🚀 Starting HIGH-SPEED migration to Neon...');
    ...
    for (const [table, rows] of Object.entries(data)) {
        ...
        await neonAdapter.pool.query(`DROP TABLE IF EXISTS ${table} CASCADE`);
        ...
        await neonAdapter.pool.query(createSql);
        ...
        for (let i = 0; i < uniqueRows.length; i += chunkSize) {
            ...
            try {
                await neonAdapter.pool.query(insertSql, values);
            } catch (err) {
        ...
        if (keys.includes('id')) {
            try {
                await neonAdapter.pool.query(`SELECT setval(pg_get_serial_sequence('${table}', 'id'), (SELECT MAX(id) FROM ${table}))`);
            } catch (e) {}
        }
